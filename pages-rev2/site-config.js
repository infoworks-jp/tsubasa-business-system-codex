(function () {
  ...
  async function renderExecutive(){
    ...
    try{
      const api=window.rev2Api,[boot,histRows,ticketRows]=await Promise.all([api('/api/bootstrap'),api('/api/historical-monthly'),api('/api/monthly')]),active=boot.active_month;
      ...
      <div class="grid2" style="margin-top:12px"><div class="panel"><h2>③ 同月4年比較・長期原票系列</h2><table><thead><tr><th>年</th><th>売上</th><th>客数</th><th>客単価</th></tr></thead><tbody>${sameHtml}</tbody></table></div><div class="panel"><h2>④ 2026年 夏3か月・2系列比較</h2><div class="notice"><b>長期原票系列と券売機確定系列は別物です。</b> 黙って置き換えません。</div><div class="scroll"><table><thead><tr><th>月</th><th>原票売上</th><th>原票客数</th><th>原票客単価</th><th>券売機売上</th><th>券売機客数</th><th>券売機客単価</th><th>原票前年比</th></tr></thead><tbody>${summerHtml}</tbody></table></div></div></div>
      <div class="panel" style="margin-top:12px"><h2>⑤ みんなでこの順にやる</h2><div class="insight"><b>1.</b> 20〜23時へ人員集中。深夜2時台は採算を毎月検証。</div><div class="insight"><b>2.</b> 9月の商品別データを確認して、売れている商品・ビール・低回転品を確定する。</div><div class="insight"><b>3.</b> 売上・客数・客単価を前年同月とセットで見る。</div><div class="insight"><b>4.</b> 値上げは基本ラーメンとトッピングの整合を崩さない。</div></div>`;
    }catch(e){host.innerHTML='<div class="panel notice ng">みんなのトップの読込に失敗しました。<br>'+String(e.message||e)+'</div>'}
  }

  function showNoProductData(tab){const title=tab==='beer'?'ビール・セット':tab==='abc'?'ABC分析':'商品別・全商品',c=state?.quality?.coverage||{},partial=c.product_status==='partial',rows=Array.isArray(state?.products)?state.products:[];if(!partial){document.getElementById('host').innerHTML=`<div class="panel"><h2>${title}</h2><div class="notice ng"><b>未登録・分析不可</b><br>空欄や0円は「売れていない」という意味ではありません。商品別原票が登録されるまで通常グラフは出しません。</div></div>`;return}const sorted=[...rows].sort((a,b)=>Number(b.sales)-Number(a.sales)),beer=sorted.filter(x=>/ビール/.test(x.name)),shown=tab==='beer'?beer:sorted;const detail=shown.length?table(['商品','登録済み出数','登録済み売上'],shown.map(x=>[x.name,Number(x.qty).toLocaleString(),yen(x.sales)]),true):'<div class="notice ng">登録済み明細を取得できません。</div>';document.getElementById('host').innerHTML=`<div class="panel"><h2>${title}</h2><div class="notice ng"><b>${c.operating_days}営業日中${c.product_days}日分の原票を登録済み</b><br>下表は登録済み日だけの参考集計です。月全体の順位・構成比・ABC判定は、全営業日分がそろうまで確定しません。</div><div style="margin-top:12px">${detail}</div></div>`}
