(function () {
  ...
  function showNoProductData(tab){const title=tab==='beer'?'ビール・セット':tab==='abc'?'ABC分析':'商品別・全商品',c=state?.quality?.coverage||{},partial=c.product_status==='partial';document.getElementById('host').innerHTML=`<div class="panel"><h2>${title}</h2><div class="notice ng"><b>${partial?`${c.operating_days}営業日中${c.product_days}日分のみ・分析不可`:'未登録・分析不可'}</b><br>空欄や0円は「売れていない」という意味ではありません。商品別原票が全営業日分そろうまで、ランキング・ABC分析・ビール集計の通常グラフは出しません。</div></div>`}
